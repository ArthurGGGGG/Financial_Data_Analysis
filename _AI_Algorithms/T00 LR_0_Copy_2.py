...
#import matplotlib.pyplot as plt
#arr = df.to_numpy()
#a_list = df['Financial Ratio'].tolist()
#print(a_list)
#b_list = df['Adj Close'].tolist()
#print(b_list)
...
##z = np.polyfit(np.asarray(x).astype(float), y, 2)
#z = np.polyfit(x, y, 2)
#p = np.poly1d(z)
#plt.plot(x, p(x))


## plt.scatter(df.x, df.y) # EXCEPTION THROWN 
...
# * POWER
# * MOVING AVERAGE
## POTENTENTIALY USE CV (COMPUTER VISION) AI TECHNIQUES TO EXTRACT RELEVANT DATA TO MSFT XLSX AND RETRIEVE EQ.
